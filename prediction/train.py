...
=============================================
Entrena un regresor XGBoost para predecir puntos Fantasy.
...
def _fit_model(model, X_train, y_train, X_val, y_val, verbose: int = 0):
...
def walk_forward_validate(
    ...
    for k in range(min_predict, max_j + 1):
        ...
        model = _make_xgboost()
        model = _fit_model(model, X_train, y_train, X_val, y_val)
        ...
        logger.info(
            "  Fold j%d: train=%d, pred=%d -> MAE=%.2f RMSE=%.2f R2=%.3f",
            k,
            len(train_data),
            len(test_data),
            mae,
...
def train_final_model(
    ...
    Usa la última jornada como validación para early stopping.
    """
    ...
    model = _make_xgboost()
    model = _fit_model(model, X_train, y_train, X_val, y_val, verbose=50)
    return model


def evaluate_oof(oof_df: pd.DataFrame) -> dict:
    """Evalúa métricas globales sobre predicciones OOF."""
    if oof_df.empty:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Entrena el modelo xP (xgboost)")
    parser.parse_args()
    ...
    oof_df, fold_metrics = walk_forward_validate(df, feature_cols)
    ...
    print(f"\n  Métricas OOF globales ({len(oof_df)} predicciones, {len(fold_metrics)} folds):")
    ...
    fi = feature_importance(final_model, feature_cols)
    ...
    save_model(final_model, save_metrics, feature_cols)

    oof_path = DATA_DIR / f"oof_{MODEL_TYPE}.csv"
    oof_df.to_csv(oof_path, index=False)
    print(f"\n  Predicciones OOF guardadas: {oof_path}")

    last_j = oof_df["jornada"].max()
    muestra = oof_df[oof_df["jornada"] == last_j].nlargest(15, "xP")
    print(f"\n  Muestra OOF — Jornada {int(last_j)} ({MODEL_TYPE}):")
